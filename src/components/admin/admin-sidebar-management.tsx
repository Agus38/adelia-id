...
export function AdminSidebarManagement() {
  const { adminSidebarMenuItems, isLoading } = useAdminSidebarMenuConfig();
  ...
  useEffect(() => {
    if (adminSidebarMenuItems) {
      setMenuState(adminSidebarMenuItems);
    }
  ...
  const handleDelete = (id: string) => {
    if (id === 'admin-dashboard') {
        toast({ title: "Tidak Diizinkan", description: "Item menu 'Dasbor Admin' tidak dapat dihapus.", variant: "destructive"});
        return;
    }
  ...
  const handleSaveChanges = async () => {
    setIsSaving(true);
    try {
        await saveAdminSidebarMenuConfig(menuState);
        toast({
            title: "Perubahan Disimpan!",
            description: "Konfigurasi sidebar admin telah berhasil diperbarui di database."
        })
    } catch (error) {
         toast({
  ...
  return (
    <div>
          ...
          <TableBody>
             {isLoading ? (
                ...
                <TableRow key={item.id}>
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleMove(index, 'up')} disabled={index <= 1 || item.id === 'admin-dashboard'}>
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleEdit(item)}>
                     ...
                     <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button variant="destructive" size="sm" disabled={item.id === 'admin-dashboard'}>Hapus</Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                            <AlertDialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isNew ? 'Tambah Menu Sidebar Admin' : `Edit Menu: ${selectedMenuItem?.title}`}</DialogTitle>
            <DialogDescription>
             {isNew ? "Buat item menu baru untuk sidebar admin." : "Ubah detail item menu di bawah ini."}
            </DialogDescription>
          </DialogHeader>
          {selectedMenuItem && (
            <div className="grid gap-4 py-4">
                 <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input 
                        ...
                        disabled={!isNew}
                        placeholder="cth: menu-baru-admin"
                    />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input 
                        ...
                        className="col-span-3" 
                        placeholder="/admin/path-url"
                         disabled={selectedMenuItem.id === 'admin-dashboard'}
                    />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
