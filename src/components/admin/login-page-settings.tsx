...
import { useLoginPageConfig, saveLoginPageConfig, type LoginPageConfig } from '@/lib/menu-store';
import { toast } from '@/hooks/use-toast';
import { Loader2, UploadCloud } from 'lucide-react';
import { Skeleton } from '../ui/skeleton';

export function LoginPageSettings() {
  const { loginPageConfig, isLoading } = useLoginPageConfig();
  const [localConfig, setLocalConfig] = useState<LoginPageConfig | null>(null);
  ...
  useEffect(() => {
    if (loginPageConfig) {
      setLocalConfig(JSON.parse(JSON.stringify(loginPageConfig)));
    }
  }, [loginPageConfig]);
  ...
  const handleSaveChanges = async () => {
    ...
    setIsSaving(true);
    try {
      await saveLoginPageConfig(localConfig);
      toast({
        title: 'Perubahan Disimpan!',
        description: 'Pengaturan halaman login telah berhasil diperbarui.',
      });
    } catch (error) {
      toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Ubah gambar yang ditampilkan di halaman login. Rekomendasi rasio 2:3.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Contoh: office team"
          />
