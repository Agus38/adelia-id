/**
 * @fileOverview This file contains the schema definitions and the main Genkit prompt
...
export const AssistantInputSchema = z.object({
  history: z.array(MessageSchema).describe('The conversation history, including an initial system message with user context.'),
  appContext: z.object({
...
// Define the Genkit prompt. This is just the definition, not the execution.
export const nexusAssistantPrompt = ai.definePrompt({
    name: 'nexusAssistantPrompt',
    system: systemPrompt,
    tools: [getDeveloperInfo, getCurrentTime],
    // The history will be passed dynamically during execution.
    // The Zod schema for history is defined in AssistantInputSchema.
    input: { schema: z.object({ history: z.array(MessageSchema) }) },
});
